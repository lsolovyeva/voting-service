...
class DishServiceTest {
    ...
    void addNewDish() {
        ...
        Dish dish = dishService.addNewDish(TestData.testDish, TestData.RESTAURANT_ID);
    ...
    void NotAddNewDishWhenAlreadyPresent() {
        when(restaurantRepository.findById(TestData.RESTAURANT_ID)).thenReturn(Optional.of(TestData.testRestaurant));
        when(restaurantRepository.findByRestaurantIdAndDishName(any(), any())).thenReturn(Optional.of(TestData.testRestaurant));
        assertThrows(UnsupportedOperationException.class, () -> dishService.addNewDish(TestData.testDish, TestData.RESTAURANT_ID));
    }
    ...
    void NotAddNewDishWhenRestaurantNotFound() {
        ...
        assertThrows(EntityNotFoundException.class, () -> dishService.addNewDish(TestData.testDish, TestData.RESTAURANT_ID));
    }
    ...
    void updateDish() {
        ...
        when(dishRepository.save(any())).thenReturn(TestData.testDish);
        assertTrue(dishService.updateDish(TestData.testDish, TestData.DISH_ID));
    }

    @Test
    void notUpdateDishWhenNotFound() {
        when(dishRepository.findById(any())).thenReturn(Optional.empty());
        assertThrows(EntityNotFoundException.class, () -> dishService.updateDish(TestData.testDish, TestData.DISH_ID));
    }
}
